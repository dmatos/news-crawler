...
queryListParams <-emolQueryListExample()
#comando para o que eu procurar como se estivesse no site

query_string <- 'crisis+humanitaria'

queryListParams$query<-query_string
...
out <- tryCatch(
	{
		emolQuery2(queryListParams, start_date_input = start_date, end_date_input = end_date)
	}, error = function(cond){
...
resultado <-search_keywords(keywords_list_pt, 'emol')
